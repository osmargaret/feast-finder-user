...
function VendorDashboardPage() {
  ...
  const allAvailableVendors = useMemo(() => {
    ...
    if (vendorCtx.profile) {
      ...
      const myVendor = {
        ...
        avatar:
          vendorCtx.profile.images[0] ||
          "https://images.unsplash.com/photo-1556910103-1c02745aae4d?w=100",
        cover:
          vendorCtx.profile.bannerUrl ||
  ...
  const myMeals = useMemo(
    () => menu.meals.filter((m) => m.vendorId === vendorId),
    [menu.meals, vendorId],
  );
  ...
  const myPosts = useMemo(
    () => blog.posts.filter((p) => p.vendorId === vendorId),
    [blog.posts, vendorId],
  );
    ...
    return (
      <>
        <PageHero
          ...
          subtitle="You need a vendor account to manage a kitchen."
        />
        <section className="section">
          <div className="container-mm">
            <div className="card-mm mx-auto max-w-md p-10 text-center space-y-3">
              <Link to="/signin" className="btn-primary inline-flex w-full justify-center">
                Sign in as Vendor
              </Link>
              <Link to="/vendor-signup" className="btn-ghost inline-flex w-full justify-center">
    ...
    return (
      <>
        <PageHero
          ...
          subtitle="Your customer account cannot access the vendor dashboard."
        />
        <section className="section">
          <div className="container-mm">
            <div className="card-mm mx-auto max-w-md p-10 text-center space-y-3">
              <p className="text-sm font-semibold text-muted-foreground">
                Signed in as <span className="font-black text-foreground">{auth.user.name}</span>{" "}
                (customer account). To manage a kitchen, create a separate vendor account.
              </p>
              <Link to="/vendor-signup" className="btn-primary inline-flex w-full justify-center">
                Create a vendor account
              </Link>
  ...
  return (
    <>
      <PageHero
        ...
        subtitle="Manage every corner of your kitchen."
      />
      <section className="section">
        <div className="container-mm">
          <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
              ...
              <div className="mb-4 flex flex-col gap-4">
                ...
                >
                  <div className="flex items-center gap-2">
                    <div
                      className={`h-2 w-2 rounded-full ${vendorProfile.profile?.isOpen ? "bg-primary animate-pulse shadow-[0_0_8px_var(--color-primary)]" : "bg-muted-foreground"}`}
                    />
                    {vendorProfile.profile?.isOpen ? "Kitchen Open" : "Kitchen Closed"}
                  </div>
                  <div
                    className={`relative h-5 w-10 rounded-full transition-colors ${vendorProfile.profile?.isOpen ? "bg-primary" : "bg-muted-foreground/30"}`}
                  >
                    <div
                      className={`absolute top-1 h-3 w-3 rounded-full bg-white transition-all ${vendorProfile.profile?.isOpen ? "right-1" : "left-1"}`}
                    />
                  </div>
                </button>
            ...
            <div className="min-w-0">
              ...
              {active === "launchpad" && (
                <LaunchpadTab vendor={vendor} meals={myMeals} onAction={(tab) => setActive(tab)} />
              )}
              ...
              {active === "blog" && (
                <BlogTab vendorId={vendorId} authorEmail={auth.user.email} posts={myPosts} />
              )}
    ...
    <>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-2xl font-black">
          Menu items{" "}
          <span className="text-sm font-bold text-muted-foreground">({meals.length})</span>
        </h2>
        <button onClick={() => setEditing("new")} className="btn-primary inline-flex">
            ...
            <li key={m.id} className="card-mm overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <button
                    onClick={() => menu.removeMeal(m.id)}
                    className="inline-flex items-center gap-1 rounded-full border border-border px-3 py-1.5 text-xs font-bold text-destructive hover:bg-destructive/5"
                  >
          ...
          <li key={o.id} className="card-mm p-5">
            ...
            <div className="mt-3 flex items-center justify-between border-t border-border pt-3 text-sm">
              <span className="text-muted-foreground">
                Deliver to {o.address.street}, {o.address.city}
              </span>
    ...
    return Array.from(map.entries())
      .map(([email, msgs]) => {
        ...
        return {
          ...
          msgs,
          last,
          customerName: userMsg?.fromName ?? "Customer",
      ...
      {threads.map((t) => {
        ...
        return (
          <li
            key={t.email}
            id={`thread-${t.email}`}
            className={`card-mm p-4 ${unread ? "border-primary/40 bg-primary/5" : ""} ${selectedEmail === t.email ? "ring-2 ring-primary ring-offset-2" : ""}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-extrabold">
                  {t.customerName}{" "}
                  <span className="text-xs font-normal text-muted-foreground">· {t.email}</span>
                </p>
                <p className="text-[11px] font-bold text-muted-foreground">
                  {t.msgs.length} message(s) · last {new Date(t.last.ts).toLocaleString()}
                </p>
              </div>
            </div>
            <div className="mt-3 max-h-64 space-y-2 overflow-y-auto pr-1">
              {t.msgs.map((m: any) => (
                ...
                >
                  <div
                    className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${m.from === "vendor" ? "bg-primary text-primary-foreground" : "bg-secondary"}`}
                  >
                    <p>{m.body}</p>
                    <p
                      className={`mt-0.5 text-[10px] ${m.from === "vendor" ? "text-primary-foreground/75" : "text-muted-foreground"}`}
                    >
            ...
            <div className="mt-3 flex gap-2">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    ...
                    if (text) {
                      ctx.reply(t.vendorId, t.email, text, vendorName);
                      setDraft({ ...draft, [t.email]: "" });
              ...
              >
                <Check className="h-3.5 w-3.5" /> Reply
              </button>
            </div>
          </li>
        );
  ...
  const top = [...meals]
    .map((m) => ({
      ...
      sold: orders.reduce(
        (s, o) =>
          s +
          o.items.filter((i: any) => i.mealId === m.id).reduce((a: number, i: any) => a + i.qty, 0),
        0,
      ),
    }))
    .filter((x) => x.sold > 0)
    ...
    <>
      ...
      <div className="card-mm mt-6 p-5">
        <h3 className="mb-3 flex items-center gap-2 text-lg font-extrabold">
          <TrendingUp className="h-4 w-4 text-primary" /> Best sellers
        </h3>
        {top.length === 0 ? (
  ...
  return (
    <>
      ...
      <div className="card-mm mt-6 overflow-hidden">
        <div className="border-b border-border bg-secondary/40 px-5 py-3 text-xs font-bold uppercase text-muted-foreground">
          Recent transactions
        </div>
        {rows.length === 0 ? (
                ...
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="p-3 text-right">
                      <span
                        className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase ${r.status === "delivered" ? "bg-primary/10 text-primary" : "bg-secondary text-foreground/70"}`}
                      >
                        {r.status}
                      </span>
                    </td>
...
function BlogTab({
  ...
  return (
    <>
      ...
      {posts.length === 0 ? (
        <div className="card-mm p-10 text-center">
          <p className="text-sm text-muted-foreground">No blog posts yet. Share your story!</p>
        </div>
      ) : (
      ...
      <div className="card-mm p-6 bg-white/40">
        ...
        <div className="relative w-full group pb-2">
          ...
          >
            ...
            <path
              d="M0,80 L66.6,60 L133.3,75 L200,40 L266.6,55 L333.3,20 L400,35 V100 H0 Z"
              fill="url(#grad)"
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
            ...
            {pendingOrders.length === 0 ? (
              ...
              <ul className="space-y-3">
                {pendingOrders.slice(0, 3).map((o) => (
                  <li
                    key={o.id}
                    className="flex items-center justify-between rounded-2xl bg-secondary/30 p-4 ring-1 ring-border"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          {o.items.length} items · {formatPrice(o.subtotal)}
                        </p>
                      </div>
                    </div>
                    <span
                      className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${o.status === "pending" ? "bg-orange-500/10 text-orange-600 animate-pulse" : "bg-primary/10 text-primary"}`}
                    >
                      {o.status}
                    </span>
                  </li>
        ...
        <div className="space-y-6">
          <div className="card-mm p-6 h-fit">
            ...
            <button onClick={() => onAction("messages")} className="btn-ghost w-full mt-4 text-xs">
              Open Messages
            </button>
          </div>
          ...
          <div className="card-mm p-6 bg-gradient-to-br from-primary/5 to-secondary/50 border border-primary/10">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Adding photos to your menu items increases order conversion by up to{" "}
...
function LaunchpadTab({
  ...
  const hasBankDetails = !!profile?.bankDetails?.accountNumber;
  const hasDeliveryAreas = (profile?.deliveryAreas?.length ?? 0) > 0;
  const hasCategories = (profile?.categories?.length ?? 0) > 0;
  const hasBrandVisuals = !!(profile?.logoUrl || profile?.bannerUrl);

  const goToSettings = () =>
    navigate({ to: "/settings" } as any).then?.(() => {}) ||
    (window.location.href = "/settings?view=vendor");
  ...
  const steps: {
  ...
  }[] = [
    {
      ...
      desc: "Your vendor profile is live on Feast Finder.",
      done: true,
    },
    {
      label: "Add your logo & banner",
      desc: "Help customers recognise your brand at a glance.",
      done: hasBrandVisuals,
      action: () => (window.location.href = "/settings?view=vendor"),
    },
    {
      label: "Set your food categories",
      desc: "Tell customers what type of food you serve.",
      done: hasCategories,
      action: () => (window.location.href = "/settings?view=vendor"),
    },
    {
      label: "Add your first menu items",
      desc: "Customers can't order if there's nothing on the menu.",
      done: hasMeals,
      action: () => onAction("menu"),
    },
    {
      label: "Configure delivery areas",
      desc: "Set locations and fees so customers know where you deliver.",
      done: hasDeliveryAreas,
      action: () => (window.location.href = "/settings?view=vendor"),
    },
    {
      label: "Setup payout account",
      desc: "Add your bank details to receive weekly settlements.",
      done: hasBankDetails,
      action: () => onAction("income"),
    },
  ];

  const completedCount = steps.filter((s) => s.done).length;
  const progress = Math.round((completedCount / steps.length) * 100);
  const isFullySetUp = completedCount === steps.length;
    ...
    <div className="space-y-6">
      ...
      <div className="card-mm overflow-hidden bg-primary p-8 text-primary-foreground shadow-xl relative isolate">
        ...
        <h2 className="text-3xl font-black italic tracking-tight">
          {isFullySetUp
            ? `You're all set, ${vendor.name}! 🚀`
            : `Welcome to Feast Finder, ${vendor.name}! 🎉`}
        </h2>
        <p className="mt-2 text-primary-foreground/80 font-medium">
          {isFullySetUp
            ? "Your kitchen is fully configured. Sit back and watch the orders roll in."
            : "Complete the steps below to get your kitchen ready to receive orders."}
        </p>
      </div>
      ...
      <div className="card-mm p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-xl font-extrabold italic">Setup Checklist</h3>
          <span
            className={`text-sm font-black px-3 py-1 rounded-full ${
              isFullySetUp ? "bg-emerald-500/10 text-emerald-600" : "bg-primary/10 text-primary"
            }`}
          >
        ...
        {/* Progress bar */}
        <div className="mb-6 h-2.5 overflow-hidden rounded-full bg-secondary/60">
          <div
        ...
        <ul className="space-y-3">
          {steps.map((step, idx) => (
            <li
              key={idx}
              className={`flex items-center justify-between gap-4 rounded-2xl border-2 px-5 py-4 transition-all ${
                step.done
            ...
            >
              <div className="flex items-center gap-4">
                {/* Step indicator */}
                <div
                  className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl text-sm font-black transition-all ${
                    step.done
              ...
              {!step.done && step.action && (
                <button
                  onClick={step.action}
                  className="shrink-0 rounded-xl bg-primary px-4 py-2 text-[11px] font-black uppercase tracking-widest text-primary-foreground shadow-md shadow-primary/20 hover:opacity-90 transition-opacity"
                >
                  Start →
                </button>
              )}
      ...
      {!hasMeals && (
        <div className="card-mm flex flex-col items-center justify-center p-12 text-center border-dashed border-2 bg-secondary/20 border-primary/20">
          ...
          <p className="mt-2 text-sm text-muted-foreground max-w-sm font-medium">
            Customers can't order if there's nothing on the menu. Add your best dishes now to start
            selling on Feast Finder.
          </p>
          <button
            onClick={() => onAction("menu")}
            className="btn-primary mt-8 px-10 py-4 text-base shadow-2xl shadow-primary/30 hover:scale-105 transition-transform"
          >
    ...
    <div className="space-y-6">
      ...
      <div className="card-mm p-6 bg-gradient-to-br from-primary/5 to-transparent">
        ...
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-4">
          <div className="sm:col-span-1">
            <label className="block text-[10px] font-black uppercase text-muted-foreground mb-1.5 ml-1">
              Promo Code
            </label>
            <input
              value={code}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {myCoupons.map((c) => (
          <div
            key={c.id}
            className={`card-mm p-5 border-2 ${c.active ? "border-primary/20 bg-primary/5" : "border-border opacity-60"}`}
          >
            <div className="flex items-center justify-between mb-3">
              ...
              >
                <div
                  className={`absolute top-1 h-4 w-4 rounded-full bg-white transition-all ${c.active ? "right-1" : "left-1"}`}
                />
              </button>
            </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {reviews.length === 0 ? (
          <div className="card-mm p-12 text-center text-muted-foreground">
            No reviews yet. Your customers' feedback will appear here.
          </div>
        ) : (
          reviews.map((r: any) => (
                  ...
                  <div>
                    ...
                    <div className="flex gap-0.5 mt-0.5">
                      {[...Array(5)].map((_, i) => (
                        <StarIcon
                          key={i}
                          className={`h-3 w-3 ${i < r.rating ? "text-orange-400 fill-orange-400" : "text-muted-foreground/30"}`}
                        />
                      ))}
                    </div>
                  </div>
